pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("taherquazi/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'taherquazi') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull taherquazi/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d taherquazi/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
